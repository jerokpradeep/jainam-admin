<template>
    <div>
        ...
        <div class="p-4">
            <div class="card p-4">
                <form @submit.prevent="getVendorData()" class="gap-2 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-6">
                    <div>
                        <p class="primaryColor pb-1 text-sm">User ID</p>
                        <input type="text" v-model="userId" maxlength="10" class="border w-full h-10 rounded focus:outline-0 px-4 text-xs">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="text-sm">
                            ...
                            <td class="border-r text-center">
                                <button class="px-4 py-1 text-xs rounded" :class="i.authorization_status == '1' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'">{{ i.authorization_status == '1' ? 'Active' : 'UnActive' }}</button>
                            </td>
                            <td class=" text-center">
                                <button class="px-4 py-1 text-xs rounded bg-blue-500 text-white " @click="handleAuthorize(i)">{{ i.authorization_status == '1' ? 'Un Authorize' : 'Authorize' }}</button>
                            </td>
                            </tr>
                        </tbody>
                    </table>
