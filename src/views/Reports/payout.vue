<style>
/* tr,td,th{
    @apply border border-collapse text-center p-2
} */
</style>
<template>
    ...
    <div class="card p-4">
      <div class="mb-6 gap-2 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-6">
        <div>
          ...
          <input type="date" v-model="fromDate" class="border w-full h-10 rounded focus:outline-0 px-4 text-xs"
            :max="new Date().toISOString().slice(0, 10)" />
        </div>
        <div>
          <p class="primaryColor pb-1 text-sm">To Date</p>
          <input type="date" v-model="toDate" :disabled="!fromDate" :max="new Date().toISOString().slice(0, 10)"
            :min="fromDate" class="border w-full h-10 rounded focus:outline-0 px-4 text-xs" />
        </div>
        <div class="flex items-end">
          ...
          <button class="bg-blue-500 text-white h-10 w-[120px] ml-6 cursor-pointer rounded text-xs"
      ...
      <div class="border rounded mt-2" v-if="payOutDetails.length">
        <table class="w-full">
          ...
          <tbody class="text-sm">
            <tr v-for="(i, id) in payOutDetails" :key="id" class="border-b">
  ...
  methods: {
    ...
    getDetails() {
      if (this.fromDate && this.toDate) {
        ...
        this.$store.dispatch("accessLog/callPayoutReport", json);
      } else {
        this.$notify({
          group: "auth",
          type: "error",
